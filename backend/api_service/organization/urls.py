...
organizationkyc_list = views.OrganizationKYCViewSet.as_view(
    {"get": "list", "post": "create"}
)
organizationkyc_detail = views.OrganizationKYCViewSet.as_view({"put": "update"})
...
urlpatterns = [
    ...
    path(
        "<int:organization_id>/scan-organization",
    ...
    path(
        "manual-entry-visitors-first-step/",
        views.manualEntryVisitorsFirstStep.as_view(),
        name="manual-entry-visitors-first-step",
    ),
    path(
        "manual-entry-visitors-second-step/",
        views.manualEntryVisitorsSecondStep.as_view(),
        name="manual-entry-visitors-second-step",
    ),
    path(
        "approve-visitor/", views.ApproveVisitorView.as_view(), name="approve-visitor"
    ),
    path(
    ...
    path(
        "<int:organization_id>/settings",
        views.OrganizationSettingsView.as_view(),
        name="organization-settings",
    ),
    path("organization-list-name", views.OrganizationNameListView.as_view()),
    path(
        "report-of-org/<int:organization_id>/",
        views.OrganizationVisitStatisticsView.as_view(),
    ),
    path("organizationkyc/", organizationkyc_list, name="organizationkyc-list"),
    path(
        "organizationkyc/<int:pk>/",
        organizationkyc_detail,
        name="organizationkyc-detail",
    ),
    path("devices/", views.device_list_view, name="device-list-create"),
    path(
        "purpose/", views.PurposeViewSet.as_view({"get": "list"}), name="purpose-list"
    ),
    path(
        "organization-content/",
        views.OrganizationContentAPIView.as_view(),
        name="organization-content-api",
    ),
    path(
        "ads-banners/list", views.AdsBannerListAPIView.as_view(), name="ads-banner-list"
    ),
    path(
        "ads-banners/create",
        views.AdsBannerCreateAPIView.as_view(),
        name="ads-banner-list",
    ),
    path(
        "ads-banners/<int:pk>/",
        views.AdsBannerDetailAPIView.as_view(),
        name="ads-banner-detail",
    ),
    path(
        "<int:organization_id>/kyc/list", views.ListKYCView.as_view(), name="list-kyc"
    ),
    path(
