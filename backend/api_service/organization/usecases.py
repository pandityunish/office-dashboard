...
class CreateOrganizationBranchUseCase(BaseUseCase):
    ...
    def _factory(self):
        if OrganizationBranch.objects.filter(
            mobile_no=self._data.get("mobile_no")
        ).exists():
        ...
        OrganizationBranch.objects.create(organization=self.instance, **self._data)
...
class CreateOrganizationKYCUseCase(BaseUseCase):
    ...
    def send_notification(organization):
        ...
        text = (
            f"Thank You! {organization.organization.full_name}"
    ...
    def add_organization_address(self, organization: OrganizationKYC):
        ...
        organization.organization.address = (
            f"{district}, {municipality}, {city_village_area}, "
        )
    ...
    def _factory(self):
        try:
            ...
            registration_certificate_data = self._data.pop(
            ...
            organization_kyc = OrganizationKYC.objects.create(
                organization=self.instance, **self._data
            )

            (
                organization_kyc.logo.save(
                    self.extension(logo_data),
                    ContentFile(base64.b64decode(logo_data)),
                    save=False,
                )
                if logo_data
                else None
            ...
            (
                organization_kyc.registration_certificate.save(
                    self.extension(registration_certificate_data),
                    ContentFile(base64.b64decode(registration_certificate_data)),
                    save=False,
                )
                if registration_certificate_data
                else None
            )
            (
                organization_kyc.PAN_VAT_certificate.save(
                    self.extension(registration_certificate_data),
                    ContentFile(base64.b64decode(pan_vat_certificate_data)),
                    save=False,
                )
                if pan_vat_certificate_data
                else None
            )
            (
                organization_kyc.licenses.save(
                    self.extension(licenses_data),
                    ContentFile(base64.b64decode(licenses_data)),
                    save=False,
                )
                if licenses_data
                else None
            ...
            (
                organization_kyc.citizenship.save(
                    self.extension(citizenship_data),
            ...
            (
                organization_kyc.passport.save(
                    self.extension(passport_data),
                    ContentFile(base64.b64decode(passport_data)),
                    save=False,
                )
                if passport_data
                else None
            ...
            (
                organization_kyc.driving_license.save(
                    self.extension(driving_license_data),
                    ContentFile(base64.b64decode(driving_license_data)),
                    save=False,
                )
                if driving_license_data
                else None
            )
            ...
            raise ValidationError({"error": "Kyc already exists"})

        # Add social media links
        for social_media_data in social_media_links_data:
            social_media_link = OrganizationKYCSocialMediaLink.objects.create(
                **social_media_data
            )
        ...
        for doc_data in documents_data:
            ...
            if doc_file_data:
                doc.file.save(
                    self.extension(doc_file_data),
                    ContentFile(base64.b64decode(doc_file_data)),
                    save=True,
                )
...
class ScanOrganizationUseCase(BaseUseCase):
    ...
    def _factory(self):
        visitor_history = OrganizationVisitHistory.objects.create(
            ...
            full_name=(
                self.request.user.full_name if self.request.user.full_name else None
            ),
            email=self.request.user.email if self.request.user.email else None,
            mobile_number=(
                self.request.user.mobile_number
                if self.request.user.mobile_number
                else None
            ),
        ...
        def visit_message():
            if self.instance.approve_visitor_before_access:
                return f"Hi {self.instance.full_name}, you have a New Visitor {self.request.user.full_name} visited your Office."
            else:
                return f"Hi {self.instance.full_name}, you have a New Visitor {self.request.user.full_name}. Please check and approve."
        ...
        notification_data_for_visitor = {
            "notification_type": "other",
            "audience": "visitor",
            "title": "Thanks for visiting",
            "message": f"Thanks for visiting {self.instance.full_name} Organization.",
        }

        send_notification(
            user=self.request.user,
            notification_data=notification_data_for_visitor,
        )
    ...
    def response_data(self, visitor_history):
        visit_data = {
            ...
            "organization_full_name": (
                self.instance.full_name if self.instance.full_name else None
            ),
            "organization_address": (
                self.instance.address if self.instance.address else None
            ),
...
class OrganizationSettingsUseCase(BaseUseCase):
    ...
    def _factory(self):
        if (
            self._data.get("approve_visitor_before_access")
            == self.instance.approve_visitor_before_access
        ):
            raise ValidationError(
                {
            ...
            self.instance.approve_visitor_before_access = self._data.get(
