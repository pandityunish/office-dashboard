...
export default function Visitorlist() {
  ...
  useEffect(() => {
    fetchCustomers();
  }, [currentPage, customersPerPage, selectedDate, endSelectedDate, isFilter]);

  const fetchCustomers = (e = "") => {
    getCustomersList({
      toast: toast,
      searchtext: e,
      setvisitor: handleSetCustomers,
  ...
  const handleSetCustomers = (data) => {
    if (data.results && data.results.customer_info.length > 0) {
      setCustomers(data.results.customer_info);
  ...
  const handleDeleteCustomer = () => {
    deleteCustomer({ toast: toast, id: customerId }).finally(() => {
      setCustomers(null);
      fetchCustomers();
      handleClose();
    });
  ...
  const endEntry = Math.min(currentPage * customersPerPage, totalCustomers);
  ...
  const startEntries = (currentPage - 1) * customersPerPage + 1;
  const endEntries = Math.min(currentPage * customersPerPage, totalCustomers);
  ...
  return (
    <div className="lg:w-full w-[1367px]  mt-10 rounded-xl p-7 shadow-lg bg-white font-inter">
      ...
      <div className="flex justify-between">
        <h1 className="font-bold text-2xl leading-9 ">Customer List</h1>
      ...
      <div className="flex items-end mt-5 justify-between">
        <p className="font-normal text-xs">
          Showing {startEntry} to {endEntry} of {totalCustomers} entries
        </p>
